...
async function register (req, res) {
    ...
    try{
// 1. check to see if user already exist
...
// 2. if they dont (new user ... good!) encrypt the password

const encryptedPassword = await bcrypt.hash(req.body.password, 10)
console.log(req.body.password)
console.log(encryptedPassword)

// 3. Add a new user to the database (with encrypted password)
...
async function login (req, res) {
   ...
   if (!foundUser) {
    ...
    // 2. Check if the password provided by user matches the one in DB
    ...
    // 3. Generate a JWT toekn and return it to the user
